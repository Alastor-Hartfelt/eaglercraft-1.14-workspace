...
public class ParticleManager implements IFutureReloadListener {
      ...
      CompletableFuture<?>[] completablefuture = Registry.PARTICLE_TYPE.keySet().stream().map((p_215228_4_) -> {
         return CompletableFuture.runAsync(() -> {
            try {
               ...
               System.err.println("FATAL ASYNC ERROR IN PARTICLE MANAGER: " + p_215228_4_);
               t.printStackTrace();
            }
         }, backgroundExecutor);
      }).toArray((p_215239_0_) -> {
          ...
          reloadProfiler.endStartSection("bindSpriteSets");
           TextureAtlasSprite textureatlassprite = this.atlas.getSprite(MissingTextureSprite.getLocation());
            map.forEach((p_215227_2_, p_215227_3_) -> {
               ParticleManager.AnimatedSpriteImpl animatedSprite = this.sprites.get(p_215227_2_);
               if (animatedSprite != null) {
                  ImmutableList<TextureAtlasSprite> immutablelist = p_215227_3_.isEmpty() ? ImmutableList.of(textureatlassprite) : ImmutableList.copyOf(p_215227_3_.stream().map(this.atlas::getSprite).collect(Collectors.toList()));
                  animatedSprite.setSprites(immutablelist);
               }
            });
           this.sprites.forEach((key, anim) -> {
              if (!map.containsKey(key)) {
                 anim.setSprites(ImmutableList.of(textureatlassprite));
              }
           });
   ...
   private void loadTextureLists(IResourceManager manager, ResourceLocation particleId, Map<ResourceLocation, List<ResourceLocation>> textures) {
      ...
      try (
         ...
         System.err.println("Failed to load description for particle " + particleId);
   ...
   class AnimatedSpriteImpl implements IAnimatedSprite {
      ...
      private AnimatedSpriteImpl() {
       ...
       public TextureAtlasSprite get(int particleAge, int particleMaxAge) {
          if (this.field_217594_b == null || this.field_217594_b.isEmpty()) {
             return ParticleManager.this.atlas.getSprite(MissingTextureSprite.getLocation());
          }
      ...
      public void setSprites(List<TextureAtlasSprite> p_217592_1_) {
         this.field_217594_b = p_217592_1_ == null || p_217592_1_.isEmpty() ? ImmutableList.of(ParticleManager.this.atlas.getSprite(MissingTextureSprite.getLocation())) : ImmutableList.copyOf(p_217592_1_);
      }
   }
